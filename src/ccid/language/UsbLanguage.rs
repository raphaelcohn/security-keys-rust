...
impl From<Language> for UsbLanguage
{
	#[inline(always)]
	fn from(value: Language) -> Self
	{
		Self::from(value.lang_id())
	}
}

impl From<u16> for UsbLanguage
{
	#[inline(always)]
	fn from(language_identifier: u16) -> Self
	{
		Self::parse(language_identifier)
	}
}
...
{
	#[inline(always)]
	pub(super) fn convert_languages(languages: Vec<Language>) -> Vec<Self>
	{
		let mut converted = Vec::with_capacity(languages.len());
		for language in languages
		{
			converted.push(Self::from(language));
		}
		converted
	}
	
	#[inline(always)]
	fn parse(language_identifier: u16) -> Self
	{
		const PRIMARY_LANGUAGE_MASK: u16 = 0x03FF;
		const SUB_LANGUAGE_MASK: u16 = 0xFC00;
		
		use self::UsbLanguage::*;
