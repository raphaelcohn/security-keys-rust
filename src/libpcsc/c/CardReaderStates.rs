...
impl<'card_reader_names, UserData> CardReaderStates<'card_reader_names, UserData>
{
	...
	pub(crate) fn get_reader_state(&self, index: usize) -> (CardReaderEventName<'card_reader_names>, Option<&UserData>, InsertionsAndRemovalsCount, StateChanged, CardReaderState)
	{
		...
		let (insertion_and_removal_count, event_state) =
		{
		...
		(reader_name, user_data, insertion_and_removal_count, has_changed, reader_event_state)
	}
	...
	fn get_status_change(&mut self, timeout: Timeout, context: SCARDCONTEXT) -> Result<(), CardReaderStatusChangeError>
	{
		...
		match result
		{
			...
			SCARD_E_UNKNOWN_READER => Err(UnknownCardReader),
			SCARD_E_READER_UNAVAILABLE => Err(UnavailableCardReader),
			
			SCARD_E_CANCELLED => Err(Cancelled),
			SCARD_E_TIMEOUT => Err(TimedOut),
			
			SCARD_E_NO_MEMORY => Err(Communication(OutOfMemory)),
			SCARD_E_NO_SERVICE => Err(Communication(ThereIsNoDaemonRunningOrConnectionWithTheDaemonCouldNotBeEstablished)),
			SCARD_F_COMM_ERROR => Err(Communication(InternalCommunications)),
