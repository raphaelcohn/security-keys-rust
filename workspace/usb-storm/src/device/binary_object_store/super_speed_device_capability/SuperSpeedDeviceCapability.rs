...
impl SuperSpeedDeviceCapability
{
	...
	#[inline(always)]
	pub(super) fn parse(device_capability_bytes: &[u8]) -> Result<Self, SuperSpeedDeviceCapabilityParseError>
	{
		...
		{
			let bmAttributes = device_capability_bytes.u8(0);
		...
		let wSpeedsSupported = device_capability_bytes.u16(1);
		...
		let lowest_speed_supporting_full_functionality =
		{
			...
			let bFunctionalitySupport = device_capability_bytes.u8(3);
		...
		let bU1DevExitLat = device_capability_bytes.u8(4);
		if unlikely!(bU1DevExitLat >= 0x0B)
		{
			return Err(HasReservedU1DeviceExitLatency { bU1DevExitLat })
		}
		
		let bU2DevExitLat = device_capability_bytes.u16(5);
