...
/// Output terminal entity descriptor.
#[derive(Debug, Clone, Eq, PartialEq, PartialOrd, Ord, Hash)]
#[derive(Deserialize, Serialize)]
#[serde(deny_unknown_fields)]
pub struct OutputTerminalEntity
{
...
impl Entity for OutputTerminalEntity
{
	type EntityIdentifier = TerminalEntityIdentifier;
	
	type ParseError = OutputTerminalEntityParseError;
		...
		(
			...
			(
				Self
				{
					...
					description:
					{
						let dead_or_alive = device_connection.find_string(entity_body.u8(entity_index::<8>())).map_err(InvalidDescriptionString)?;
						return_ok_if_dead!(dead_or_alive)
					},
					
					terminal_type: OutputTerminalType::parse(bLengthUsize, entity_body)?,
					
					source: entity_body.optional_non_zero_u8(entity_index::<7>()),
				}
			)
		)
	}
...
impl WithSourceEntity for OutputTerminalEntity
{
	#[inline(always)]
...
impl TerminalEntity for OutputTerminalEntity
{
	type TerminalType = OutputTerminalType;
...
impl OutputTerminalEntity
{
	const MinimumBLength: usize = OutputSpecificTerminalType::MinimumBLength as usize;
