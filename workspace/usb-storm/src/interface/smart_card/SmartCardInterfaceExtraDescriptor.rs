...
impl SmartCardInterfaceExtraDescriptor
{
	...
	fn parse(protocol: SmartCardProtocol, descriptor_body: &[u8]) -> Result<Self, SmartCardInterfaceExtraDescriptorParseError>
	{
		...
		let iso_7816_protocols = WrappedBitFlags::from_bits_truncate(descriptor_body.u32(descriptor_index::<6>()));
		let features = crate::interface::smart_card::Features::parse(descriptor_body.u32(descriptor_index::<40>()), iso_7816_protocols).map_err(Features)?;
		Ok
		(
			Self
			{
				...
				firmware_version: descriptor_body.version(descriptor_index::<2>()).map_err(Version)?,
				
				maximum_slot_index: descriptor_body.u8(descriptor_index::<4>()),
				
				voltages_supported: WrappedBitFlags::from_bits_truncate(descriptor_body.u8(descriptor_index::<5>())),
	
				iso_7816_protocols,
			
				default_clock_frequency: descriptor_body.kilohertz(descriptor_index::<10>()),
			
				inclusive_maximum_clock_frequency: descriptor_body.kilohertz(descriptor_index::<14>()),
				
				number_of_clock_frequencies_supported: descriptor_body.optional_non_zero_u8(descriptor_index::<18>()),
				
				default_data_rate: descriptor_body.baud(descriptor_index::<19>()),
				
				inclusive_maximum_data_rate: descriptor_body.baud(descriptor_index::<23>()),
				
				number_of_data_rates_supported: descriptor_body.optional_non_zero_u8(descriptor_index::<27>()),
				
				maximum_ifsd_for_protocol_t_1: if iso_7816_protocols.contains(Iso7816Protocol::T1)
				{
					Some(descriptor_body.u32(descriptor_index::<28>()))
				}
				else
				{
					None
				...
				synchronization_protocols: WrappedBitFlags::from_bits_truncate(descriptor_body.u32(descriptor_index::<32>())),
				
				mechanical_features: WrappedBitFlags::from_bits_truncate(descriptor_body.u32(descriptor_index::<36>())),
				
				features,
				
				maximum_message_length: descriptor_body.u32(descriptor_index::<44>()),
				...
				lcd_layout: LcdLayout::from(descriptor_body.u16(descriptor_index::<50>())),
				
				pin_support: WrappedBitFlags::from_bits_truncate(descriptor_body.u8(descriptor_index::<52>())),
				
				maximum_slots_that_can_be_simultaneously_used:
				{
					let raw = descriptor_body.u8(descriptor_index::<53>());
					if unlikely!(raw == 0)
					{
						new_non_zero_u8(1)
					}
			...
			(
				T0ProtocolUnconfiguredClasses
				{
					apdu_get_response:
					{
						...
						let bClassGetResponse = descriptor_bytes.u8(descriptor_index::<48>());
					...
					extended_apdu_envelope: if level_of_exchange.is_extended_apdu_level()
					{
						...
						// * 0xFF commonest value
						let bClassEnvelope = descriptor_bytes.u8(descriptor_index::<49>());
