...
impl Interface
{
	...
	pub(super) fn parse(libusb_interface: &libusb_interface, string_finder: &StringFinder, interface_index: u8, maximum_supported_usb_version: Version) -> Result<DeadOrAlive<(InterfaceNumber, Self)>, InterfaceParseError>
	{
		...
		{
			let parsed_interface_number = return_ok_if_dead!(Self::parse_alternate_setting(alternate_settings_slice, string_finder, interface_index, alternate_setting_index, &mut alternate_settings, maximum_supported_usb_version)?);
