...
impl Interface
{
	...
	#[inline(always)]
	pub(super) fn parse(libusb_interface: &libusb_interface, device_connection: &DeviceConnection, reusable_buffer: &mut ReusableBuffer, interface_index: u8, maximum_supported_usb_version: Version, speed: Option<Speed>) -> Result<DeadOrAlive<(InterfaceNumber, Self)>, InterfaceParseError>
	{
		...
		{
			let dead_or_alive = Self::parse_alternate_setting(alternate_settings_slice, device_connection, reusable_buffer, interface_index, 0, &mut alternate_settings, maximum_supported_usb_version, speed)?;
			return_ok_if_dead!(dead_or_alive)
		};
		
		for alternate_setting_index in 1 .. number_of_alternate_settings.get()
		{
			let parsed_interface_number =
			{
				let dead_or_alive = Self::parse_alternate_setting(alternate_settings_slice, device_connection, reusable_buffer, interface_index, alternate_setting_index, &mut alternate_settings, maximum_supported_usb_version, speed)?;
				return_ok_if_dead!(dead_or_alive)
			};
