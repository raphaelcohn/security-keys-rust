...
impl DescriptorParser for DeviceFirmwareUpgradeInterfaceAdditionalDescriptorParser
{
	...
	fn parse_descriptor(&mut self, _string_finder: &StringFinder, bLength: u8, descriptor_type: DescriptorType, remaining_bytes: &[u8]) -> Result<Option<DeadOrAlive<(Self::Descriptor, usize)>>, Self::Error>
	{
		...
		let maximum_detach_time_out_milliseconds = descriptor_body.u16(adjust_descriptor_index::<3>());
		let maximum_number_of_bytes_per_control_write_transaction = descriptor_body.u16(adjust_descriptor_index::<5>());
		...
		{
			Some(descriptor_body.version(adjust_descriptor_index::<7>()).map_err(Version)?)
		}
		else
		{
			None
