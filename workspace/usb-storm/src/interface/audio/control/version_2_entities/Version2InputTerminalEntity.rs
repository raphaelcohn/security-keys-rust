...
impl Entity for Version2InputTerminalEntity
{
	...
	fn parse(entity_body: &[u8], string_finder: &StringFinder) -> Result<DeadOrAlive<Self>, Self::ParseError>
	{
		...
		let bmControls = entity_body.u16_unadjusted(adjusted_index::<14>());
		...
		(
			...
			(
				Self
				{
					...
					copy_protect_control: Control::parse_u16(bmControls, 0, InputTerminalCopyProtectControlInvalid)?,
					
					connector_control: Control::parse_u16(bmControls, 1, InputTerminalConnectorControlInvalid)?,
					
					overload_control: Control::parse_u16(bmControls, 2, InputTerminalOverloadControlInvalid)?,
					
					cluster_control: Control::parse_u16(bmControls, 3, InputTerminalClusterControlInvalid)?,
					
					underflow_control: Control::parse_u16(bmControls, 4, InputTerminalUnderflowControlInvalid)?,
					
					overflow_control: Control::parse_u16(bmControls, 5, InputTerminalOverflowControlInvalid)?,
					
					description:
					{
						let description = string_finder.find_string(entity_body.u8_unadjusted(adjusted_index::<16>())).map_err(InvalidDescriptionString)?;
