...
impl EntityDescriptors for Version2EntityDescriptors
{
	...
	fn parse_entity_body(&mut self, bDescriptorSubtype: u8, string_finder: &StringFinder, entity_descriptors_bytes: &[u8], bLength: u8) -> Result<DeadOrAlive<bool>, EntityDescriptorParseError<Self::Error>>
	{
		...
		{
			INPUT_TERMINAL => parse_entity_descriptor::<_, 17>(string_finder, entity_descriptors_bytes, bLength, &mut self.input_terminal)?,
			
			OUTPUT_TERMINAL => parse_entity_descriptor::<_, 12>(string_finder, entity_descriptors_bytes, bLength, &mut self.output_terminal)?,
			...
			CLOCK_SOURCE =>
			{
				parse_entity_descriptor::<_, 8>(string_finder, entity_descriptors_bytes, bLength, &mut self.source_clock)?
			}
			...
			CLOCK_MULTIPLIER =>
			{
				parse_entity_descriptor::<_, 7>(string_finder, entity_descriptors_bytes, bLength, &mut self.multiplier_clock)?
			}
			
			SAMPLE_RATE_CONVERTER =>
			{
				parse_entity_descriptor::<_, 8>(string_finder, entity_descriptors_bytes, bLength, &mut self.sampling_rate_converter_unit)?
			}
