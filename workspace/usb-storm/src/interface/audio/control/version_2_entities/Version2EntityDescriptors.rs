...
impl EntityDescriptors for Version2EntityDescriptors
{
	...
	fn parse_entity_body(&mut self, bDescriptorSubtype: u8, string_finder: &StringFinder, entity_descriptors_bytes: &[u8], bLength: u8, entity_identifiers: &mut HashSet<EntityIdentifier>) -> Result<DeadOrAlive<bool>, EntityDescriptorParseError<Self::Error>>
	{
		...
		let dead_or_alive = match bDescriptorSubtype
		{
			...
			CLOCK_SOURCE =>
			{
				parse_entity_descriptor::<_, 8>(string_finder, entity_descriptors_bytes, bLength, entity_identifiers, &mut self.source_clock)?
			}
			...
			CLOCK_MULTIPLIER =>
			{
				parse_entity_descriptor::<_, 7>(string_finder, entity_descriptors_bytes, bLength, entity_identifiers, &mut self.multiplier_clock)?
			}
			
			SAMPLE_RATE_CONVERTER =>
			{
				parse_entity_descriptor::<_, 8>(string_finder, entity_descriptors_bytes, bLength, entity_identifiers, &mut self.sampling_rate_converter_unit)?
			}
