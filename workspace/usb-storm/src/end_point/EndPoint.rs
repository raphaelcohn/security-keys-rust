...
{
	#[inline(always)]
	pub(crate) fn parse(end_point_descriptor: &libusb_endpoint_descriptor, interface_class: InterfaceClass, maximum_supported_usb_version: Version, device_connection: &DeviceConnection, speed: Option<Speed>, end_points: &mut WrappedIndexMap<EndPointNumber, Self>) -> Result<DeadOrAlive<()>, EndPointParseError>
	{
		...
		let common = return_ok_if_dead!(EndPointCommon::parse(end_point_descriptor, interface_class, device_connection, bLength, &mut transfer_type)?);
