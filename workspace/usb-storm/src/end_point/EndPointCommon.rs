...
impl EndPointCommon
{
	...
	#[inline(always)]
	pub(super) fn parse(end_point_descriptor: &libusb_endpoint_descriptor, interface_class: InterfaceClass, device_connection: &DeviceConnection, bLength: u8, transfer_type: &mut Either<Option<NonZeroU8>, (Direction, DirectionalTransferType)>) -> Result<DeadOrAlive<Self>, EndPointParseError>
	{
		...
		let descriptors = Self::parse_descriptors(device_connection, end_point_descriptor, interface_class, transfer_type, maximum_packet_size).map_err(CouldNotParseEndPointAdditionalDescriptor)?;
